  ...
  return (
    <>
          ...
          {(() => {
            const bannerContent = (
              <div className="absolute inset-0 flex items-center justify-between px-4 md:px-12 gap-4 shadow-2xl group/billboard overflow-hidden bg-black">
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-red-600 via-pink-500 to-red-700 z-0 pointer-events-none"></div>
                  ...
                  <div className="flex flex-col items-start text-left relative z-10">
                    {activeNotice?.text ? (
                      <>
                        ...
                        <h2 className="text-white font-serif text-base md:text-2xl font-black tracking-wide drop-shadow-md leading-none truncate max-w-[300px] sm:max-w-full">
                          {activeNotice.text}
                        </h2>
                      </>
                    ) : (
                ...
                <div className="relative z-10 flex-shrink-0 flex items-center gap-3">
                  <span className="hidden md:block text-white/80 font-bold text-xs animate-bounce-x mr-2">Click to Claim</span>
                  <div className="bg-white text-[#E60023] px-4 md:px-6 py-2 md:py-2.5 rounded-full font-black text-xs md:text-sm shadow-xl hover:scale-105 transform transition-all active:scale-95 flex items-center gap-2 border border-red-100">
                    {activeNotice?.link ? 'GRAB DEAL' : 'INFO'}
      ...
      {(isNoticeActive && !isBillboard) && (
        <div className={`absolute bottom-0 left-0 right-0 h-[3px] z-50 pointer-events-none transition-all duration-700 origin-center ${
          isVisible ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-50'
        }`}>
          {/* Vivid Base Line */}
          <div className="absolute inset-x-0 bottom-[-1px] h-[3px] bg-gradient-to-r from-transparent via-[#E60023] to-transparent"></div>
          {/* Expansive Light Burst Effect */}
          <div className="absolute inset-x-0 bottom-[-1px] h-[6px] bg-gradient-to-r from-transparent via-[#E60023] to-transparent blur-[6px] animate-pulse"></div>
          <div className="absolute inset-x-0 bottom-[-1px] h-[15px] bg-gradient-to-r from-transparent via-red-500/40 to-transparent blur-[15px] opacity-70 animate-pulse delay-75"></div>
        </div>
      )}

      {/* NORMAL FLOW WRAPPER: Constrains standard content and anchors layout height */}
      <div className="flex flex-col items-center justify-center text-center w-full relative min-h-[70px] max-w-[650px] overflow-visible">
        ...
        <div className={`w-full h-full flex flex-col items-center justify-center transition-all duration-500 ease-in-out ${
          ...
          <div className="flex flex-col items-center">
            <h1 
              className={`${titleFont || 'font-serif'} text-2xl md:text-4xl font-bold tracking-wide flex items-center gap-2 justify-center`}
              style={titleColor ? { color: titleColor } : {}}
            >
              {title}
            </h1>
            <p 
              className={`${subtitleFont || 'font-sans'} text-[11px] sm:text-md md:text-sm mt-1 md:mt-1.5 animate-in fade-in duration-700 ${!subtitleColor ? 'opacity-70' : ''}`}
              ...
              const content = (
                <div className="w-full flex items-center justify-between md:justify-center gap-2 md:gap-6 py-1 max-w-full overflow-visible px-1.5 md:px-6 group">
                  ...
                  {activeNotice?.imageUrl && (
                    isVideoMedia ? (
                      <video
                        ...
                        src={activeNotice.imageUrl}
                        autoPlay loop muted playsInline
                        className="w-10 h-10 md:w-12 md:h-12 rounded-xl object-cover shadow-md flex-shrink-0 bg-black ring-2 ring-red-500/20 hidden sm:block"
                      />
                    ) : (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={activeNotice.imageUrl} alt="" className="w-10 h-10 md:w-12 md:h-12 rounded-xl object-cover shadow-md flex-shrink-0 bg-black text-transparent ring-2 ring-red-500/20 hidden sm:block" />
                    )
                  )}

                  <div className="bg-[#E60023] text-white text-[9px] md:text-[11px] font-black uppercase px-1.5 md:px-2 py-0.5 rounded shadow-md flex-shrink-0 tracking-widest animate-bounce-subtle">
                    {activeNotice?.link ? '🔥 DEAL' : 'AD'}
                  </div>
                  ...
                  <div className="flex-1 min-w-0 flex justify-center px-1">
                    <h2 className="font-sans text-[13px] sm:text-lg md:text-xl text-[#C4001D] font-black tracking-wide drop-shadow-[0_1px_3px_rgba(230,0,35,0.1)] leading-tight whitespace-normal break-words text-center">
                      {activeNotice?.text}
                    </h2>
                  </div>

                  {activeNotice?.link && (
                    <div className="bg-[#E60023] text-white text-[10px] md:text-sm font-black rounded-full px-2 md:px-4 py-1 md:py-2 shadow-lg flex items-center gap-1 border-0 flex-shrink-0 transition-all transform hover:scale-105 hover:bg-red-700">
